...
2. Framework commands listed in `cobratree/classify.go.tmpl`'s `frameworkCommands` set (`about`, `agent-context`, `api`, `auth`, `completion`, `doctor`, `feedback`, `help`, `profile`, `version`, `which`) are skipped because a typed equivalent already covers the capability (`about`, `agent-context`, `api`) or the command is non-functional via MCP (`auth`, `completion`, `doctor`, `version`, `feedback`, `profile`, `which`, `help`).
3. `cmd.Annotations["mcp:hidden"] = "true"` opts out one command at a time, for reasons `frameworkCommands` cannot express at top-level-name granularity: a command superseded by a hand-registered typed tool (`search`), a not-yet-implemented stub that always errors (the HTML page-mode `sync` stub, below), a destructive bulk operation (`learnings purge`), or a domain command needing human-in-the-loop input.

`sql` and `context` are MCP-only typed tools registered directly in `mcp_tools.go.tmpl`; no `sql` or `context` Cobra command exists, so the walker never sees them and they were never candidates to skip. The CLI equivalent of the `sql` tool is `analytics`. `search` does exist as a Cobra command and is excluded via rule 3, not rule 2 — it is not in `frameworkCommands`. `cobratree/walker.go.tmpl`'s `s.GetTool(toolName) != nil` check guards against a shell-out tool name colliding with a typed tool, but it is not what keeps `search` from double-registering: `walk()` filters `mcp:hidden` commands out before that check is reached.

Store-population commands stay exposed: `sync`, `stale`, `orphans`, `reconcile`, `load`, `export`, `import`, `workflow`, `analytics`. The exposed `sync` is the real spec-driven one (`sync.go.tmpl`, or `graphql_sync.go.tmpl` for GraphQL specs); it is only hidden when `generator.go` substitutes `sync_stub.go.tmpl` for a predominantly HTML page-mode spec (`visionData.HTMLSyncStub`), because that stub always returns an error. The typed `sql` and `search` tools return empty until `sync` populates the store. When in doubt, leave it exposed.
...
The repo-root `.gitattributes` sets `* text=auto eol=lf` so Go write-back tooling and checkout agree on LF. On a working tree checked out before that line landed, `core.autocrlf=true` still smudges CRLF and a single `go fmt` rewrites every formatted file's endings — leaving ~150 files staged-looking with an EMPTY content diff, which a `git add -A` would commit as a phantom EOL diff and which blocks `git merge` with "local changes would be overwritten". Fix the tree with `git add --renormalize .` rather than formatting around it, and stage edited files by exact path when in doubt.
Always use relative paths for build output. Never build to `/tmp` or another shared absolute path; use `./cli-printing-press`.
When invoking the generator by hand during development — smoke-testing a change, dogfooding a spec, anything outside `go test` — call the repo build by its relative path (`./cli-printing-press ...`), never a bare `cli-printing-press` / `printing-press` resolved from `PATH`. A stale globally `go install`-ed binary from an earlier session shadows your working tree and produces failures that look like generator bugs but are version skew: an `unknown command` for a subcommand the current source does have (or once had and removed), or a check failing against output the current source does not emit. Confirm which binary you are running with `<binary> version` before trusting a surprising failure, and re-run `go install ./cmd/cli-printing-press ./cmd/printing-press` when the global install needs to reflect the repo.
...
Never update goldens just to make a failing check pass. Run `scripts/golden.sh update` only when the behavior change is intentional, then inspect the diff and explain it in your final response. When the failing-case count is far larger than what you changed, or a `.diff` shows paired removals and additions of byte-identical text, stop — that is a CRLF working copy, not drift. Reproduce in a clean `git worktree` of the same commit first; if the clean count is lower, `update` is forbidden. See [`docs/GOLDEN.md`](docs/GOLDEN.md) for the decision rubric, fixture conventions, and failure handling.
...
- `docs/SPEC-EXTENSIONS.md` - Canonical reference for Printing Press-specific OpenAPI `x-*` extensions, plus the internal-YAML doctor probe fields (`auth.verify_path`, `auth.verify_query`, `health_check_path`). Update it when `internal/openapi/parser.go` adds or changes an `Extensions["x-*"]` lookup, or when a doctor probe field changes
...
- **Fork merge path (this repo, `origin` = SearchActions/cli-printing-press):** `main` has no branch protection and Mergify is not active here. Land normal feature/fix PRs by squash-merging the PR yourself once CI is green: `gh pr merge <n> --squash --repo SearchActions/cli-printing-press`. Never commit to `main` directly — always a `feat/`/`fix/` branch + PR, even when you merge it yourself. The `ready-to-merge` label and the `Mergify Merge Protections` check do nothing on the fork.
- **Upstream merge path (only when opening a PR against `mvanhorn/cli-printing-press`):** normal PRs land through the Mergify queue — add `ready-to-merge` and let Mergify rebase/test/merge; do not use the merge button once `Mergify Merge Protections` is required on upstream `main`. The repo-root `.mergify.yml` and `.github/CODEOWNERS` (owners `@tmchow`/`@mvanhorn`, `Greptile Review`) are upstream's gate, synced in from upstream — leave them unedited.
- release-please runs on the fork (`.github/workflows/release.yml`) and opens/updates a release PR collecting merged conventional commits. Merge that release PR (directly, after CI passes) only when you intend to cut a release — see [`docs/RELEASE.md`](docs/RELEASE.md).
...
Set `auth.verify_path` explicitly in the spec (OpenAPI: `x-auth-verify-path`) for any API with a partner or app tier alongside customer keys (Aircall, Stripe, Slack, HubSpot). Left empty, the generator derives the probe from path shape alone and can pick a partner-only endpoint, making `doctor` report `WARN Credentials: scope-limited (HTTP 403)` against valid credentials — a spec gap that reads as a credential bug. See [`docs/SPEC-EXTENSIONS.md`](docs/SPEC-EXTENSIONS.md) "Doctor Probe Fields".
