...
// Reads the copy through the linked modernc.org/sqlite driver rather than an
// external sqlite3 binary, so nothing resolved from PATH runs in the credential
// path; host_key is plaintext so no decryption is needed.
func inspectCookiesForDomain(cookiesDB, domainPattern string, requiredCookies []string) (count int, requiredCount int, missing []string) {
	...
	if err := db.QueryRow(
		"SELECT COUNT(*) FROM cookies WHERE host_key LIKE ?", domainPattern,
	).Scan(&count); err != nil {
		return 0, 0, append([]string{}, requiredCookies...)
	}
	...
	names := make([]string, 0, len(requiredCookies))
	for _, name := range requiredCookies {
	...
	rows, err := db.Query(
		"SELECT DISTINCT name FROM cookies WHERE host_key LIKE ? AND name IN ("+
...
func detectCookieTool() (cookieTool, error) {
	...
	// Resolve, never run: executing a bare PATH-resolved name to learn whether
	// it exists hands code execution to anything shadowing that name on PATH,
	// and this runs before any cookie is read. The python probe above is the
	// exception — module importability has no test but running it.
	if _, err := exec.LookPath("cookies"); err == nil {
		return cookieTool{name: "cookies"}, nil
	}
	if _, err := exec.LookPath("cookie-scoop"); err == nil {
