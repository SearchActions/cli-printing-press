...
import (
	...
	"mcp-stdio-golden-pp-cli/internal/client"
	"mcp-stdio-golden-pp-cli/internal/cliutil"
	"mcp-stdio-golden-pp-cli/internal/config"
	"mcp-stdio-golden-pp-cli/internal/store"
)
...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  mcp-stdio-golden-pp-cli doctor
  mcp-stdio-golden-pp-cli doctor --json
  mcp-stdio-golden-pp-cli doctor --fail-on warn
  mcp-stdio-golden-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// The MCP server is a local subprocess, so there is no URL to
			// reach and no credential to validate. The only real health
			// signal is whether the server starts and answers a call, which
			// also exercises the prerequisites it needs on this machine.
			if c, clientErr := flags.newClient(); clientErr != nil {
				report["api"] = fmt.Sprintf("client init error: %s", clientErr)
			} else {
				probeStart := time.Now()
				if _, probeErr := c.QueryTool(cmd.Context(), client.MCPStdioReadyTool, map[string]any{}); probeErr != nil {
					report["api"] = fmt.Sprintf("MCP server unreachable: %s", probeErr)
				} else {
					report["api"] = fmt.Sprintf("ok — MCP server answered %s in %s", client.MCPStdioReadyTool, time.Since(probeStart).Round(time.Millisecond))
				}
			}
			if len(client.MCPStdioForwardEnv) > 0 {
				missing := []string{}
				for _, name := range client.MCPStdioForwardEnv {
					if os.Getenv(name) == "" {
						missing = append(missing, name)
					}
				}
				if len(missing) == 0 {
					report["mcp_server_env"] = fmt.Sprintf("all %d set", len(client.MCPStdioForwardEnv))
				} else {
					// A WARN, not a FAIL: these configure the server, and most
					// have defaults. Their absence explains a server-side error
					// that would otherwise look like a CLI bug.
					report["mcp_server_env"] = fmt.Sprintf("WARN unset: %s (the MCP server may fall back to defaults)", strings.Join(missing, ", "))
				}
			}
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("mcp-stdio-golden-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'mcp-stdio-golden-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'mcp-stdio-golden-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "empty"
		report["hint"] = "Cache is empty; run 'mcp-stdio-golden-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'mcp-stdio-golden-pp-cli sync' to refresh."
	}
	return report
}
