...
import (
	...
	"printing-press-verify-pp-cli/internal/client"
	"printing-press-verify-pp-cli/internal/cliutil"
	"printing-press-verify-pp-cli/internal/config"
	"printing-press-verify-pp-cli/internal/store"
)
...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  printing-press-verify-pp-cli doctor
  printing-press-verify-pp-cli doctor --json
  printing-press-verify-pp-cli doctor --fail-on warn
  printing-press-verify-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "Set your API key with: export PRINTING_PRESS_VERIFY_API_KEY=\"your-token-here\""
				} else {
					authConfigured = true
			...
			if os.Getenv("PRINTING_PRESS_VERIFY_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "PRINTING_PRESS_VERIFY_API_KEY")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "PRINTING_PRESS_VERIFY_API_KEY")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["User-Agent"] = "printing-press-verify-pp-cli"
						verifyPath := "/users/self"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("printing-press-verify-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'printing-press-verify-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'printing-press-verify-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "empty"
		report["hint"] = "Cache is empty; run 'printing-press-verify-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'printing-press-verify-pp-cli sync' to refresh."
	}
	return report
}
