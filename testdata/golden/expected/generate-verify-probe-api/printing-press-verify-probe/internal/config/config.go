...
type Config struct {
	...
	PrintingPressVerifyApiKey string          `toml:"press_verify_api_key"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://api.verify-probe.example/v1",
	}
	...
	// Env var overrides
	if v := os.Getenv("PRINTING_PRESS_VERIFY_API_KEY"); v != "" {
		cfg.PrintingPressVerifyApiKey = v
		cfg.markEnvOverride("PrintingPressVerifyApiKey")
		cfg.AuthSource = "env:PRINTING_PRESS_VERIFY_API_KEY"
		cfg.CredentialSource = "env:PRINTING_PRESS_VERIFY_API_KEY"
	}
	...
	if v := os.Getenv("PRINTING_PRESS_VERIFY_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("PRINTING_PRESS_VERIFY_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "printing-press-verify-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.PrintingPressVerifyApiKey
	if token == "" {
		return ""
	}
	if c.PrintingPressVerifyApiKey == "" {
		return ""
	}
	return token
}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		PrintingPressVerifyApiKey: c.PrintingPressVerifyApiKey,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.PrintingPressVerifyApiKey = creds.PrintingPressVerifyApiKey
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.PrintingPressVerifyApiKey = token
	delete(c.envOverrides, "PrintingPressVerifyApiKey")
	c.updateFileConfigField("PrintingPressVerifyApiKey")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["PrintingPressVerifyApiKey"] {
			out.PrintingPressVerifyApiKey = c.fileConfig.PrintingPressVerifyApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "PrintingPressVerifyApiKey":
		c.fileConfig.PrintingPressVerifyApiKey = c.PrintingPressVerifyApiKey
	}
}
