...
// positive Greptile flagged on the upstream PR: GET /items returning a bare
// array of scalar IDs has no extractable primary key, so the generator's
// profiler correctly emits no store for it (profiler.IsScalarItemArray). The
// guard must not count that leg as a readable collection just because it
// pairs path-shape-wise with GET /items/{id}.
...
// CodeRabbit flagged on this fork's mirror PR: the scorecard's raw-JSON
// loader only checked a path item's own "get" key, so an OpenAPI 3.1 reusable
// Path Item Object ($ref into components.pathItems) left GETPaths empty and
// silenced the guard for a spec that legitimately declares a collection.
