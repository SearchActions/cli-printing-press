...
// direct, and sit at or above the floor; a replace directive wins over any
// require and is left alone.
func TestEnsureXSysRequire(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name: "replace directive respected as no-op",
			gomod: "module example.com/foo\n\nrequire golang.org/x/sys v0.31.0\n\n" +
				"replace golang.org/x/sys => ../local-x-sys\n",
			changed: false,
			assert: func(t *testing.T, after string) {
				assert.Contains(t, after, "replace golang.org/x/sys => ../local-x-sys")
				assert.Contains(t, after, "golang.org/x/sys v0.31.0", "require under replace is authoritative; must not move")
			},
		},
	}
