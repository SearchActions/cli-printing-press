...
func Sync(cliDir string, opts Options) (Result, error) {
	...
	// The regen emits privperms_windows.go, which imports x/sys/windows:
	// repair the require BEFORE generation (the require must exist when
	// the import lands), then tidy AFTER it — a tidy before the emission
	// sees no importer and drops the require right back out (the exact
	// cube-cli failure). Both run outside the migration-only block
	// because the surface is regenerated on every sync.
	xsysRepaired, err := ensureXSysRequire(cliDir)
	if err != nil {
		return Result{}, err
	}
	if xsysRepaired {
		fmt.Fprintf(os.Stderr, "mcp-sync: made golang.org/x/sys a direct require at %s for the regenerated private-file permission code\n", minXSysVersion)
	}
	...
	if xsysRepaired {
		if err := tidyAfterXSysRepair(cliDir); err != nil {
			return Result{}, err
		}
	}
...
// xSysModulePath / minXSysVersion identify the golang.org/x/sys require the
// regenerated cliutil needs: privperms_windows.go imports x/sys/windows, so
// a synced CLI whose go.mod predates the require (or pins below the floor)
// fails to build on Windows with "no required module provides package".
const (
	xSysModulePath = "golang.org/x/sys"
	minXSysVersion = "v0.46.0"
)

// ensureXSysRequire makes golang.org/x/sys a direct require at or above
// minXSysVersion, so the privperms files regenerated by every sync compile
// on Windows. Library CLIs sync'd before this landed have either no x/sys
// require at all (their creds guard predates it) or an indirect one below
// the floor. A replace directive on x/sys wins over any require, so it is
// respected and the go.mod is left alone. Returns true when go.mod changed.
func ensureXSysRequire(cliDir string) (changed bool, err error) {
	gomodPath := filepath.Join(cliDir, "go.mod")
	data, err := os.ReadFile(gomodPath)
	if err != nil {
	...
	// A replace directive does NOT put the module in the build graph - a
	// require still has to, or the regenerated Windows import fails to
	// build under -mod=readonly. So a replace changes WHICH version the
	// require must name; it never means "leave this alone".
	//
	//   replace x/sys => ../local        (unversioned) applies to whatever
	//                                    version is required, so the floor
	//                                    is free to win.
	//   replace x/sys v0.40.0 => ...     (version-specific) applies ONLY to
	//                                    that exact version, so requiring
	//                                    the floor instead would silently
	//                                    deactivate the replacement.
	pinnedByReplace := ""
	for _, r := range mf.Replace {
		if r.Old.Path != xSysModulePath {
			continue
		}
		if r.Old.Version != "" {
			pinnedByReplace = r.Old.Version
		}
		break
	}

	wantVersion := minXSysVersion
	if pinnedByReplace != "" {
		wantVersion = pinnedByReplace
	}

	for _, r := range mf.Require {
		if r.Mod.Path != xSysModulePath {
			continue
		}
		if !r.Indirect && r.Mod.Version == wantVersion {
			return false, nil
		}
		// Only the unpinned case may float upward; a version-specific
		// replacement has exactly one version that activates it.
		if !r.Indirect && pinnedByReplace == "" &&
			semver.IsValid(r.Mod.Version) && semver.Compare(r.Mod.Version, minXSysVersion) >= 0 {
			return false, nil
		}
		break
	}
	// Rebuild the entry rather than AddRequire: AddRequire preserves an
	// existing entry's // indirect suffix comment through Format, and the
	// point here is a DIRECT require — the import is in this CLI's own
	// regenerated source.
	...
	for _, r := range mf.Require {
		if r.Mod.Path == xSysModulePath {
			present = true
			// Keep the HIGHER of the two. An existing entry can sit above
			// the floor because some dependency needs it; assigning the
			// floor unconditionally would DOWNGRADE the one module pinned
			// here precisely because an older version was vulnerable.
			// `go mod tidy` usually repairs that, but tidyAfterXSysRepair
			// is best-effort and only warns, so a failed tidy would leave
			// the downgrade on disk.
			version := wantVersion
			// Float upward only when no version-specific replacement
			// pins us: an existing entry can legitimately sit above the
			// floor because a dependency needs it, and assigning the
			// floor unconditionally would DOWNGRADE the one module
			// pinned here precisely because an older version was
			// vulnerable.
			if pinnedByReplace == "" && semver.IsValid(r.Mod.Version) && semver.Compare(r.Mod.Version, version) > 0 {
				version = r.Mod.Version
			}
			reqs = append(reqs, &modfile.Require{
				Mod:      module.Version{Path: xSysModulePath, Version: version},
				Indirect: false,
			})
			continue
		}
	...
	if !present {
		reqs = append(reqs, &modfile.Require{
			Mod:      module.Version{Path: xSysModulePath, Version: wantVersion},
			Indirect: false,
		})
	}
	mf.SetRequire(reqs)
	rewritten, err := mf.Format()
	if err != nil {
		return false, fmt.Errorf("formatting go.mod: %w", err)
	}
	if err := writeFileAtomic(gomodPath, rewritten); err != nil {
		return false, fmt.Errorf("rewriting go.mod: %w", err)
	}
	return true, nil
}

// tidyAfterXSysRepair refreshes go.sum with the x/sys hashes the repaired
// require needs. It runs AFTER generation, so the surface is already on
// disk when it fails - but it still returns the error, because a repaired
// go.mod with no matching go.sum entry is a CLI that cannot build on
// Windows at all under -mod=readonly. Reporting success there hands the
// caller a broken tree and a warning they may never read; the message
// below carries the one-line manual fix.
func tidyAfterXSysRepair(cliDir string) error {
	...
	return fmt.Errorf("go mod tidy failed after repairing the golang.org/x/sys require: %w\n"+
		"Run 'go mod tidy' in %s before building, or the Windows build will fail on missing go.sum entries.\n%s",
		err, cliDir, out)
}
