...
func ensureXSysRequire(cliDir string) (changed bool, err error) {
	...
	for _, r := range mf.Require {
		...
		if !r.Indirect && semver.IsValid(r.Mod.Version) && semver.Compare(r.Mod.Version, minXSysVersion) >= 0 {
			return false, nil
		}
		break
	}
	...
	for _, r := range mf.Require {
		if r.Mod.Path == xSysModulePath {
			...
			reqs = append(reqs, &modfile.Require{
				Mod:      module.Version{Path: xSysModulePath, Version: minXSysVersion},
				Indirect: false,
			})
			continue
		}
...
// require needs. Best-effort by design: a tidy needs the module proxy, and
// failing the whole sync over a network hiccup would strand a correct
// go.mod with no surface regen. Warn loudly with the manual fix instead.
func tidyAfterXSysRepair(cliDir string) {
	cmd := exec.Command("go", "mod", "tidy")
	cmd.Dir = cliDir
	if out, err := cmd.CombinedOutput(); err != nil {
		fmt.Fprintf(os.Stderr, "mcp-sync: go mod tidy failed after repairing the golang.org/x/sys require (%v).\nRun 'go mod tidy' in %s before building, or the Windows build will fail on missing go.sum entries.\n%s\n", err, cliDir, out)
	}
}
