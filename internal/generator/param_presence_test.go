...
func TestGeneratedOutput_ParamPresenceGates(t *testing.T) {
	...
	// The strip is now cursor-only. Callers gate on presence before
	// populating, so an explicit empty is a value the user supplied and must
	// survive; the cursor is the sole param populated unconditionally, and it
	// is empty on the first page.
	require.Contains(t, paginatedGetBody, `if v == "" && k == cursorParam {`)
